...
int local_dlsch_encode(srslte_sch_t*                q,
                         srslte_softbuffer_tx_t*    softbuf,
                         size_t                     tbs,
                         srslte_mod_t               modulation_order,
                         uint8_t*                   data,
                         size_t                     nof_e_bits,
                         uint8_t*                   e_bits)
{
  // Prepare cbsegm
  srslte_cbsegm_t cb_segm;
  if (srslte_cbsegm(&cb_segm, (uint32_t)tbs)) {
    ERROR("Error computing Codeword segmentation for TBS=%d\n", (int)tbs);
    return SRSLTE_ERROR;
  }

  uint32_t Qm = srslte_mod_bits_x_symbol(modulation_order);
...
int local_dlsch_decode(srslte_sch_t* q, srslte_softbuffer_rx_t* sb, srslte_mod_t mod,
  uint32_t nof_e_bits, int16_t* e_bits, uint32_t nof_d_bits, uint8_t* data)
{
  uint32_t Nl = 1;
  uint32_t tb_idx = 0;
  ...
  // Prepare cbsegm
  srslte_cbsegm_t cb_segm;
  if (srslte_cbsegm(&cb_segm, (uint32_t)nof_d_bits)) {
