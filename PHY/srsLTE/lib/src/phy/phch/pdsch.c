...
int local_pdsch_codeword_decode ( srslte_pdsch_t*     q,
                                  srslte_pdsch_cfg_t* cfg,
                                  srslte_sch_t*       dl_sch,
                                  uint8_t*            data)
{
  ...
  if (softbuffer && data && cfg->grant.tb[tb_idx].nof_bits && cfg->grant.nof_re) {
    INFO("Decoding PDSCH SF: %d (CW%d -> TB%d), Mod %s, NofBits: %d, NofSymbols: %d, NofBitsE: %d, rv_idx: %d\n",
         fix_tti % 10,
         codeword_idx,
         tb_idx,
    ...
    /* Select scrambling sequence */
    srslte_sequence_t* seq =
        get_user_sequence(q, cfg->rnti, codeword_idx, fix_tti % 10, cfg->grant.tb[tb_idx].nof_bits);
    if (!seq) {
      ERROR("Error getting user sequence for rnti=0x%x\n", cfg->rnti);
      return -1;
    }
    ...
    ERROR("Invalid parameters in TB%d &softbuffer=%p &data=%p nbits=%d, nof_re=%d\n",
          codeword_idx,
          softbuffer,
          (void*)data,
          cfg->grant.tb[tb_idx].nof_bits,
...
int srslte_pdsch_codeword_encode( srslte_pdsch_t*         q,
                                  srslte_dl_sf_cfg_t*     sf,
                                  srslte_pdsch_cfg_t*     cfg,
